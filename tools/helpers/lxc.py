...
def generate_session_lxc_config(args, session):
    ...
    if not make_entry("tmpfs", session["container_xdg_runtime_dir"], options="create=dir 0 0"):
    ...
    wayland_container_socket = os.path.realpath(os.path.join(session["container_xdg_runtime_dir"], session["container_wayland_display"]))
    ...
    pulse_container_socket = os.path.join(session["container_pulse_runtime_path"], "native")
