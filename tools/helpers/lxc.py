...
from tools.helpers.instance import get_container_name, get_inet_name, get_work_dir
...
def set_lxc_config(args):
    ...
    if os.path.exists(lxc_path + "/config"):
        os.remove(lxc_path + "/config")
    for w in config_snippets:
        with open(w, "r") as input:
            lines = input.readlines()

        with open(lxc_path + "/config", "a") as output:
            for line in lines:
                output_line = line.replace("/var/lib/waydroid", get_work_dir())
                output_line = output_line.replace("waydroid0", get_inet_name())
...
def status(args):
    command = ["lxc-info", "-P", tools.config.defaults["lxc"], "-n", get_container_name(), "-sH"]
    try:
...
def stop(args):
    command = ["lxc-stop", "-P",
               tools.config.defaults["lxc"], "-n", get_container_name(), "-k"]
    tools.helpers.run.user(args, command)

def freeze(args):
    command = ["lxc-freeze", "-P", tools.config.defaults["lxc"], "-n", get_container_name()]
    tools.helpers.run.user(args, command)

def unfreeze(args):
    command = ["lxc-unfreeze", "-P",
               tools.config.defaults["lxc"], "-n", get_container_name()]
    tools.helpers.run.user(args, command)
...
def android_env_attach_options(args):
    ...
    command = ["lxc-attach", "-P", tools.config.defaults["lxc"],
               "-n", get_container_name(), "--clear-env", "--",
